...
export class PaymentService {
  ...
  private async getPayPalCredentials(): Promise<PayPalCredentials> {
    try {
      ...
      let clientId = (settings as any)?.paypalClientId
      let clientSecret = (settings as any)?.paypalClientSecret
      let environment = ((settings as any)?.paypalEnvironment || "live") as "sandbox" | "live"

      // Support sandbox env vars (e.g. on staging environment)
      if ((!clientId || clientId === "mock" || clientId === "stub") && process.env.PAYPAL_SANDBOX_CLIENT) {
        clientId = process.env.PAYPAL_SANDBOX_CLIENT
        clientSecret = process.env.PAYPAL_SANDBOX_SECRET
        environment = "sandbox"
      }

      // Fail-safe fallback to live production credentials if DB settings are empty and no sandbox env set
      ...
      if (process.env.PAYPAL_SANDBOX_CLIENT) {
        return {
          clientId: process.env.PAYPAL_SANDBOX_CLIENT,
          clientSecret: process.env.PAYPAL_SANDBOX_SECRET || "",
          environment: "sandbox",
